...
async def model_status(request: Request) -> dict[str, object]:
    ...
    active_job = next((j for j in _retrain_jobs.values() if j.get("status") in {"queued", "training"}), None)
    last_job = max(_retrain_jobs.values(), key=lambda x: str(x.get("updated_at", "")), default=None)
    return {
        "models_loaded": models_loaded,
        "detection_mode": detection_mode,
        "active_models": active_models,
        "retrain": active_job or last_job or {"status": "idle", "job_id": ""},
    }
...
async def retrain(
    ...
    _retrain_jobs[job_id] = {"job_id": job_id, "status": "queued", "updated_at": str(uuid.uuid1()), "progress": 0}
    background_tasks.add_task(_retrain_from_db, job_id, session_factory)
...
async def _retrain_from_db(
    ...
    async with _retrain_lock:
        _retrain_jobs[job_id] = {**_retrain_jobs.get(job_id, {}), "status": "training", "updated_at": str(uuid.uuid1()), "progress": 10}
        ...
        async with session_factory() as session:
            ...
            if count == 0:
                ...
                _retrain_jobs[job_id] = {**_retrain_jobs[job_id], "status": "completed", "updated_at": str(uuid.uuid1()), "progress": 100, "mode": "demo-synthetic"}
                return
        ...
        _retrain_jobs[job_id] = {**_retrain_jobs[job_id], "status": "training", "updated_at": str(uuid.uuid1()), "progress": 50}
        loop = asyncio.get_running_loop()
        try:
            result = await loop.run_in_executor(None, lambda: TrainingOrchestrator(output_dir=output_dir).run(X, y))
        except Exception as exc:
            logger.exception("Retrain job %s failed", job_id)
            _retrain_jobs[job_id] = {**_retrain_jobs[job_id], "status": "failed", "updated_at": str(uuid.uuid1()), "progress": 100, "error": str(exc)[:500]}
            return
        ...
        try:
            ...
            logger.exception("Retrain job %s: failed to write metadata", job_id)
        _retrain_jobs[job_id] = {**_retrain_jobs[job_id], "status": "completed", "updated_at": str(uuid.uuid1()), "progress": 100}
