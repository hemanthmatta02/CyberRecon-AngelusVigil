...
class AlertDispatcher:
    ...
    async def _store_event(self, scored: ScoredRequest) -> None:
        ...
        async with self._session_factory() as session:
            event = await create_threat_event(session, scored)
            # Every observed request is an operational unit of work.  Keeping
            # a linked incident from creation means SOC Operations and
            # Incident Response are always in sync with manual traffic,
            # simulations, and normal ingestion instead of relying on a
            # separate, easy-to-miss "create" step.
            session.add(
                Incident(
                    title=f"{event.severity} activity from {event.source_ip}",
                    description=f"{event.request_method} {event.request_path}",
                    severity=event.severity,
                    status="NEW",
                    source_ip=event.source_ip,
                    threat_id=event.id,
                    notes=[
                        "Created automatically from observed telemetry.",
                        f"Detection mode: {event.model_version or 'rules'}",
                    ],
                    response_actions=["operation created → NEW"],
                )
            )
