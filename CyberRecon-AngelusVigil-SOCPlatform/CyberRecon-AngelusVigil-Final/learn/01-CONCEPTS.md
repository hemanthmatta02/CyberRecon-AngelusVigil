...
This document covers the security and machine learning concepts behind CyberSentinel. These aren't textbook definitions. We'll dig into how each concept is actually implemented in the codebase and why the design choices matter.
...
CyberSentinel uses an **autoencoder** for anomaly detection. The concept is simple: train a neural network to compress and reconstruct normal traffic. If a new request can't be reconstructed well (high reconstruction error), it's probably abnormal.
...
Anomaly detection produces false positives. A legitimate user doing something unusual (bulk API calls, automated testing) will trigger alerts. That's why CyberSentinel uses an ensemble approach rather than relying on the autoencoder alone, and why it supports analyst review labels for active learning.
...
CyberSentinel uses three models with weighted voting:
...
CyberSentinel extracts two types of features that are concatenated into a single 35-dimensional vector:
...
CyberSentinel stores per-IP request history in Redis sorted sets, with the Unix timestamp as the score. This makes windowed queries efficient:
...
Rule-based detection uses handcrafted patterns to identify known attack types. CyberSentinel's rule engine is inspired by the ModSecurity Core Rule Set (CRS), the open source WAF ruleset used by OWASP.
...
Giving every rule a score of 0.90+ makes the severity system useless. CyberSentinel calibrates scores so that only the most dangerous patterns (Log4Shell, command injection) score above 0.90. Lower-confidence matches like open redirect (0.55) appropriately reflect that these patterns have higher false positive rates.
...
CyberSentinel's approach would have caught this in multiple ways:
...
The attack involved HTTP requests to internal IP addresses through the public-facing web application. CyberSentinel's rule engine specifically matches requests targeting `169.254.169.254` and other internal IP ranges (SSRF rule, score 0.70). The feature extractor also flags requests to private IP addresses via the `is_private_ip` feature.
...
While CyberSentinel monitors path and query string patterns rather than headers (it analyzes nginx access logs, not full request bodies), the behavioral anomaly detection would have flagged the post-exploitation activity: unusual paths accessed, atypical response sizes from data exfiltration, and abnormal request patterns from the attacker's tools.
...
**Essential:**
- [OWASP ModSecurity Core Rule Set](https://coreruleset.org/) - The open source WAF ruleset that inspired CyberSentinel's rule engine. Understanding CRS rules helps you write better detection patterns
...
- [Google's Rules of Machine Learning](https://developers.google.com/machine-learning/guides/rules-of-ml) - Practical lessons on when ML helps and when it doesn't. Rule #1 is "Don't be afraid to launch a product without machine learning," which is exactly why CyberSentinel starts with rules
