...
This document breaks down how CyberSentinel is designed and why each architectural decision was made. Read this before diving into the implementation.
...
**What it is:**
CyberSentinel runs alongside the target nginx server without modifying it. The backend reads nginx's access log file (read-only volume mount) and the frontend proxies API calls.

**Why we chose it:**
Zero code changes to the monitored application. You add CyberSentinel to your `compose.yml` and point it at the nginx log volume. The monitored application doesn't know it's being watched.
...
- Insider threats with direct database access to CyberSentinel itself
...
# Database
DATABASE_URL=postgresql+asyncpg://vigil:changeme@localhost:5432/cybersentinel
...
ModSecurity is the industry standard WAF engine. It runs inline (as an nginx module) and can block requests in real time. CyberSentinel runs out-of-band (reading logs after the fact) and can only detect, not block.

The trade-off: ModSecurity has zero deployment friction for blocking but is rules-only. CyberSentinel adds ML detection and behavioral analysis at the cost of being detection-only. In practice, many teams run both: ModSecurity for blocking known attacks, and a system like CyberSentinel for detecting what ModSecurity missed.

### vs. Elastic SIEM / Splunk

Elastic SIEM and Splunk are full-featured SIEMs that ingest logs from many sources, correlate events, and provide dashboards. CyberSentinel does one thing: analyze nginx HTTP traffic for threats.

The trade-off: SIEMs are more general but require significant infrastructure and tuning. CyberSentinel is purpose-built for HTTP threat detection with a trained ML ensemble. You'd use CyberSentinel as a specialized detector feeding events into a SIEM for broader correlation.
...
The trade-off: Cloud WAFs have massive training datasets and zero deployment effort, but they're opaque (you can't inspect the models) and expensive at scale. CyberSentinel is fully transparent, customizable, and free, but requires your own infrastructure and training data.
